...
int SetupCounterAndTimer(INodeMap& nodeMap)
{
    ...
    try
    {
        ...
        if (!IsReadable(ptrCounterSelector) ||
            !IsWritable(ptrCounterSelector))
        {
            cout << "Unable to get or set Counter Selector (enum retrieval). Aborting..." << endl << endl;
            return -1;
        }

        CEnumEntryPtr ptrCounter0 = ptrCounterSelector->GetEntryByName("Counter0");
        if (!IsReadable(ptrCounter0))
        {
            cout << "Unable to get Counter Selector (entry retrieval). Aborting..." << endl << endl;
            return -1;
        }
        ...
        if (!IsReadable(ptrCounterEventSource) ||
            !IsWritable(ptrCounterEventSource))
        {
            cout << "Unable to get or set Counter Event Source (enum retrieval). Aborting..." << endl << endl;
            return -1;
        }

        CEnumEntryPtr ptrCounterEventSourceMHzTick = ptrCounterEventSource->GetEntryByName("MHzTick");
        if (!IsReadable(ptrCounterEventSourceMHzTick))
        {
            cout << "Unable to get Counter Event Source (entry retrieval). Aborting..." << endl << endl;
            return -1;
        }
        ...
        if (!IsReadable(ptrCounterTriggerSource) ||
            !IsWritable(ptrCounterTriggerSource))
        {
            cout << "Unable to get or set Counter Trigger Source (enum retrieval). Aborting..." << endl << endl;
            return -1;
        }

        CEnumEntryPtr ptrCounterTriggerSourceFTW = ptrCounterTriggerSource->GetEntryByName("FrameTriggerWait");
        if (!IsReadable(ptrCounterTriggerSourceFTW))
        {
            cout << "Unable to get Counter Trigger Source (entry retrieval). Aborting..." << endl << endl;
            return -1;
        }
        ...
        if (!IsReadable(ptrCounterTriggerActivation) ||
            !IsWritable(ptrCounterTriggerActivation))
        {
            cout << "Unable to get or set Counter Trigger Activation (enum retrieval). Aborting..." << endl << endl;
            return -1;
        }

        CEnumEntryPtr ptrCounterTriggerActivationLH = ptrCounterTriggerActivation->GetEntryByName("LevelHigh");
        if (!IsReadable(ptrCounterTriggerActivationLH))
        {
            cout << "Unable to get Counter Trigger Activation (entry retrieval). Aborting..." << endl << endl;
            return -1;
        }
...
int ConfigureDigitalIO(INodeMap& nodeMap)
{
    ...
    try
    {
        ...
        if (!IsReadable(ptrLineSelector) ||
            !IsWritable(ptrLineSelector))
        {
            cout << "Unable to get or set Line Selector (enum retrieval). Aborting..." << endl << endl;
            return -1;
        }
        ...
        if (cameraModel.find(cameraFamilyBFS) != std::string::npos)
        {
            ...
            if (!IsReadable(ptrLineSelectorLine1))
            {
                cout << "Unable to get Line Selector (entry retrieval). Aborting..." << endl << endl;
                return -1;
            }
        ...
        else if (cameraModel.find(cameraFamilyOryx) != std::string::npos)
        {
            ...
            if (!IsReadable(ptrLineSelectorLine2))
            {
                cout << "Unable to get Line Selector (entry retrieval). Aborting..." << endl << endl;
                return -1;
            }
            ...
            if (!IsReadable(ptrLineMode) ||
                !IsWritable(ptrLineMode))
            {
                cout << "Unable to get or set Line Mode (enum retrieval). Aborting..." << endl << endl;
                return -1;
            }

            CEnumEntryPtr ptrLineModeOutput = ptrLineMode->GetEntryByName("Output");
            if (!IsReadable(ptrLineModeOutput))
            {
                cout << "Unable to get Line Mode (entry retrieval). Aborting..." << endl << endl;
                return -1;
            }
        ...
        if (!IsReadable(ptrLineSource) ||
            !IsWritable(ptrLineSource))
        {
            cout << "Unable to get or set Line Source (enum retrieval). Aborting..." << endl << endl;
            return -1;
        }

        CEnumEntryPtr ptrLineSourceCounter0Active = ptrLineSource->GetEntryByName("Counter0Active");
        if (!IsReadable(ptrLineSourceCounter0Active))
        {
            cout << "Unable to get Line Source (entry retrieval). Aborting..." << endl << endl;
            return -1;
        }
...
int ConfigureExposureandTrigger(INodeMap& nodeMap)
{
    ...
    try
    {
        ...
        {
            cout << "Unable to get or set Exposure Auto (enumeration retrieval). Aborting..." << endl << endl;
            return -1;
        }
        ...
        {
            cout << "Unable to get Exposure Auto (entry retrieval). Aborting..." << endl << endl;
            return -1;
        }
        ...
        if (!IsReadable(ptrTriggerSourceCounter0Start))
        {
            cout << "Unable to get trigger mode (enum entry retrieval). Aborting..." << endl;
            return -1;
        }
        ...
        {
            cout << "Unable to get or set Trigger Overlap (enum retrieval). Aborting..." << endl << endl;
            return -1;
        }

        CEnumEntryPtr ptrTriggerOverlapRO = ptrTriggerOverlap->GetEntryByName("ReadOut");
        if (!IsReadable(ptrTriggerOverlapRO))
        {
            cout << "Unable to get TriggerOverlap (entry retrieval). Aborting..." << endl << endl;
            return -1;
        }
